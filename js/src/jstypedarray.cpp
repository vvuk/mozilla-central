...
AllocateArrayBufferContents(JSContext *maybecx, uint32_t nbytes, uint8_t *initdata, void *oldptr = NULL)
{
    ...
    if (oldptr) {
        ...
        uint32_t oldnbytes = ArrayBufferObject::getElementsHeaderIntializedLength(oldheader);
        fprintf(stderr, "AllocateArrayBufferContents: realloc, oldnbytes: %d nbytes: %d new size: %d\n", oldnbytes, nbytes, size);
