...
bool
js::StartOffThreadAsmJSCompile(JSContext *cx, AsmJSCompilationData *asmData)
{
    ...
    JS_ASSERT(!state.asmJSWorkerFailed());

    AutoLockWorkerThreadState lock(cx->runtime);

    if (!state.asmWorklist.append(asmData))
        return false;
...
static void
FinishOffThreadAsmJSCompile(AsmJSCompilationData *asmData)
{
    JSRuntime *rt = asmData->mir->compartment->rt;
    JS_ASSERT(rt->workerThreadState);
    JS_ASSERT(rt->workerThreadState->isLocked());
    JS_ASSERT(asmData->lir);

    rt->workerThreadState->asmFinishedList.append(asmData);
}

// TODO: Need non-threadsafe empty variants?
...
WorkerThreadState::canStartAsmJSCompile()
{
    ...
    JS_ASSERT(isLocked());
    return (!asmWorklist.empty() && !numAsmJSFailedJobs);
}
...
WorkerThread::handleAsmJSWorkload(WorkerThreadState &state)
{
    ...
    asmData = state.asmWorklist.popCopy();
    bool success = false;
...
WorkerThread::threadLoop()
{
    ...
    while (true) {
        ...
        // TODO: This is new behavior, so suspect this for bugs.
        if (terminate) {
            state.unlock();
            return;
        }
