...
class FunctionCompiler
{
  ...
  private:
    ...
    // TODO: This probably goes in LinkAsmJS().
  ...
  public:
    ...
    void finishCallArgs(Args *args)
    {
        ...
        mirGen().setAsmJSMaxStackArgBytes(newStackBytes);
    }
...
static MIRGenerator *
GenerateFunctionMIR(JSContext *cx, ModuleCompiler &m, ModuleCompiler::Func &func, LifoAlloc &lifo)
{
    ...
    MIRGenerator *mirGen = lifo.new_<MIRGenerator>(cx->compartment, tempAlloc, graph, info);
...
static bool
CheckFunctionBodiesSequential(JSContext *cx, ModuleCompiler &m)
{
    // TODO: Here we want some AutoAssertNoSystemAlloc.
...
GetFinishedCompilation(JSContext *cx, WorkerThreadState &state, ModuleCompiler &m)
{
    AutoLockWorkerThreadState lock(cx->runtime);

    while (!state.asmJSWorkerFailed()) {
        if (!state.asmFinishedList.empty())
            return state.asmFinishedList.popCopy();
...
// Extract the original LifoAlloc fram the AsmJSCompilationData,
// clear it for reuse by another worker thread, and return it.
// Since the AsmJSCompilationData is stored inside the LifoAlloc,
// it is unreadable after calling this function.
static LifoAlloc *
RecycleUsedLifoAlloc(AsmJSCompilationData *asmData)
{
    TempAllocator &tempAlloc = asmData->mir->temp();
    LifoAlloc *lifo = asmData->mir->temp().lifoAlloc();

    tempAlloc.TempAllocator::~TempAllocator(); // Appeases useless assertions.
    lifo->releaseAll();
    return lifo;
}

static bool
CancelOutstandingJobs(JSContext *cx, WorkerThreadState &state, int32_t numCompiling)
{
    // This is failure-handling code, so it's not really allowed to fail.
    // Always returns false for the aesthetic value of its callsite.

    JS_ASSERT(numCompiling >= 0);
    if (!numCompiling)
        return false;
    ...
    numCompiling -= state.asmWorklist.length();
    state.asmWorklist.clear();

    // From the compiling tasks, eliminate those waiting for codegen.
    numCompiling -= state.asmFinishedList.length();
    state.asmFinishedList.clear();

    // Eliminate tasks that failed without adding to the finished list.
    numCompiling -= state.harvestFailedAsmJSJobs();

    // Any remaining tasks are therefore undergoing active compilation.
    JS_ASSERT(numCompiling >= 0);
    while (numCompiling > 0) {
        state.wait(WorkerThreadState::MAIN);
...
static inline bool
ParallelError(int32_t numCompiling, int32_t *out)
{
    JS_ASSERT(numCompiling >= 0);
    *out = numCompiling;
    return false;
...
static bool
DispatchParallelJobs(JSContext *cx, WorkerThreadState &state, ModuleCompiler &m,
                     LifoAlloc *lifoList, size_t numParallelJobs, int32_t *numOutstanding)
{
    state.resetAsmJSFailureState();

    int32_t numCompiling = 0;
    DebugOnly<uint32_t> numCompiled = 0;
    ...
    for (uint32_t i = 0; i < m.numFunctions(); i++) {
        ...
        LifoAlloc *lifo = NULL;
        if (i < numParallelJobs) {
            // Then the LifoAlloc was never assigned to a worker thread.
            lifo = &lifoList[i];
        } else {
            AsmJSCompilationData *asmData = GetFinishedCompilation(cx, state, m);
            if (!asmData)
                return ParallelError(numCompiling, numOutstanding);
            numCompiling--;

            // Generate code on the main thread.
            if (!GenerateAsmJSCode(m, m.function(asmData->funcNum), *asmData->mir, *asmData->lir))
                return ParallelError(numCompiling, numOutstanding);

            lifo = RecycleUsedLifoAlloc(asmData);
            numCompiled++;
        }

        // Generate MIR on the main thread.
        MIRGenerator *mir = GenerateFunctionMIR(cx, m, func, *lifo);
        if (!mir)
            return ParallelError(numCompiling, numOutstanding);

        // Perform optimizations and LIR generation on a worker thread.
        AsmJSCompilationData *asmData = lifo->new_<AsmJSCompilationData>(i, mir);
        if (!StartOffThreadAsmJSCompile(cx, asmData))
            return ParallelError(numCompiling, numOutstanding);
        numCompiling++;
    }

    // Block for all outstanding worker tasks to complete.
    while (numCompiling > 0) {
        AsmJSCompilationData *asmData = GetFinishedCompilation(cx, state, m);
        if (!asmData)
            return ParallelError(numCompiling, numOutstanding);
    ...
    JS_ASSERT(numCompiling == 0);
    JS_ASSERT(numCompiled == m.numFunctions());
    JS_ASSERT(state.asmWorklist.empty());
    JS_ASSERT(state.asmFinishedList.empty());
...
CheckFunctionBodiesParallel(JSContext *cx, ModuleCompiler &m)
{
    ...
    LifoAlloc *lifoList = m.lifo().newArrayUninitialized<LifoAlloc>(numParallelJobs);
    for (size_t i = 0; i < numParallelJobs; i++)
        new (&lifoList[i]) LifoAlloc(LIFO_ALLOC_PARALLEL_CHUNK_SIZE);

    // Good work, jobs!
    int32_t outstandingJobs;
    
    if (!DispatchParallelJobs(cx, state, m, lifoList, numParallelJobs, &outstandingJobs))
        return CancelOutstandingJobs(cx, state, outstandingJobs);
...
CheckModule(JSContext *cx, TokenStream &ts, ParseNode *fn, ScopedJSDeletePtr<AsmJSModule> *module)
{
    ...
    if (ion::js_IonOptions.parallelCompilation) {
        if (!CheckFunctionBodiesParallel(cx, m))
            return false;
    } else {
        printf("Forgot to turn on parallel.\n");
        if (!CheckFunctionBodiesSequential(cx, m))
            return false;
    }
#else
    if (!CheckFunctionBodiesSequential(cx, m))
