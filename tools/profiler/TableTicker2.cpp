...
// Pseudo backtraces are available on all platforms.  Native backtraces
// are available only on selected platforms, currently x86_64-linux and
// arm-android.  Breakpad is the only supported native unwinder.
// HAVE_NATIVE_UNWIND is set at build time to indicate whether native
// unwinding is possible on this platform.  The actual unwind mode
// currently in use is stored in sUnwindMode.
...
static
void genProfileEntry(/*MOD*/UnwinderThreadBuffer* utb,
...
void genPseudoBacktraceEntries(/*MOD*/UnwinderThreadBuffer* utb,
...
void TableTicker2::Tick(TickSample* sample)
{
  ...
  void* /* ucontext_t*, really */ ucV 
    = (sUnwindMode == UnwNATIVE || sUnwindMode == UnwCOMBINED)
        ? sample->context : NULL;
...
# endif
  ...
  assert(sUnwindMode     == UnwINVALID);
  assert(sUnwindInterval == 0);
 ...
 usage:
  ...
  LOG( "SPS: ");
  LOG( "SPS:   You can use SPS_MODE     instead of MOZ_PROFILER_MODE");
  LOG( "SPS:   You can use SPS_INTERVAL instead of MOZ_PROFILER_INTERVAL");
  LOG( "SPS: ");
 ...
 out:
  ...
  LOGF("SPS: Sampling interval = %d ms (zero means \"platform default\")",
       (int)sUnwindInterval);
  LOG( "SPS: Use env var SPS_MODE=help for further information.");
  LOG( "SPS:");
...
void mozilla_sampler_init2()
{
  ...
  // and (temporarily) register this thread for profiling
  int aLocal;
